...
fn process_input(input: &str) -> usize {
    println!("INFO: parsing starting...");
    let (seeds, almanac) = parse_input(input);
    println!("INFO: parsing done");
    println!("INFO: processing seeds starting...");
    seeds
        .into_par_iter()
        .progress()
        .map(|seed| seed.location(&almanac) as usize)
        .min()
...
fn parse_seeds(input: &str) -> Vec<Seed> {
    let seeds_list = input
        ...
        .collect::<Vec<_>>();
    let mut seeds = vec![];
    for x in (0..seeds_list.len()).step_by(2) {
        let start = seeds_list[x]
            .parse::<usize>()
            .expect("Failed to parse seed range start number.");
        let end = seeds_list[x + 1]
            .parse::<usize>()
            .expect("Failed to parse seed range start number.")
            + start;
        for y in start..end {
            seeds.push(Seed::new(y));
        }
    }
        ...
        match words[x] {
            "map:" => {
                ...
                while (x < words.len()) && (words[x].parse::<usize>().is_ok()) {
                    ...
                    let source_range_start = words[x + 1]
                        .parse::<usize>()
                        .expect("Failed to parse source range start.");
                    let range_length = words[x + 2]
                        .parse::<usize>()
