...
fn process_input(input: &str) -> usize {
    ...
    for (sr, sc) in points.clone() {
        ...
        while let Some((n, r, c)) = stack.pop() {
            ...
            for (dr, dc) in vec![(-1, 0), (1, 0), (0, -1), (0, 1)].into_iter() {
    ...
    dfs(start, &graph, &mut BTreeSet::new(), end)
}
...
fn dfs(
    ...
    for nx in graph.get(&pt).unwrap().keys() {
        ...
        max = std::cmp::max(
            max,
            dfs(*nx, graph, seen, end) + graph.get(&pt).unwrap().get(nx).unwrap(),
        );
    }
    seen.remove(&pt);
...
#.....###...###...#...#
#####################.#";
        assert_eq!(process_input(input), 154);
    }
}
