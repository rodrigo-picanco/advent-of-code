...
fn process_input(input: &str) -> usize {
    ...
    for (name, left, right) in nodes {
        schema.insert(name.clone(), State::new(name, (left, right)));
    }

    let initial = schema.get("AAA").expect("No initial state").clone();
    schema.insert(
        "initial".to_string(),
        State::new("initial".to_string(), (initial.transitions.0, initial.transitions.1)),
    );

    let mut machine = WalkMachine::new(schema);

    moves.iter().cycle().find_map(|direction| {
        machine.walk(direction.clone());
        if machine.state.name.eq("ZZZ") {
            Some(machine.steps)
        } else {
            None
        }
...
mod tests {
    ...
    fn test_count_steps() {
        ...
        schema.insert(
            "initial".to_string(),
            State::new("AAA".to_string(), ("".to_string(), "".to_string())),
        );
        schema.insert(
            "AAA".to_string(),
            State::new("AAA".to_string(), ("BBB".to_string(), "CCC".to_string())),
        );
        schema.insert(
            "BBB".to_string(),
            State::new("BBB".to_string(), ("AAA".to_string(), "CCC".to_string())),
        );

        schema.insert(
            "CCC".to_string(),
            State::new("CCC".to_string(), ("CCC".to_string(), "CCC".to_string())),
        );
        let mut machine = WalkMachine::new(schema);
    ...
    fn test_walk() {
        ...
        schema.insert(
            "AAA".to_string(),
            State::new("AAA".to_string(), ("BBB".to_string(), "BBB".to_string())),
        );
        schema.insert(
            "BBB".to_string(),
            State::new("BBB".to_string(), ("AAA".to_string(), "CCC".to_string())),
